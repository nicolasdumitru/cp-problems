...
// Submission URL: https://cses.fi/problemset/result/15542966/
// Verdict: ACCEPTED
...
private:
    auto dfs(const usize v, const usize p, std::vector<usize> &results) -> std::unordered_set<u32> {
        auto color_unordered_set = std::unordered_set<u32>({colors[v]});
        for (const auto u : adj[v]) {
            if (u == p) {
                continue;
            }
            auto u_color_unordered_set = dfs(u, v, results);
            merge_small_to_large(color_unordered_set, u_color_unordered_set);
        }
        results[v] = color_unordered_set.size();
        return color_unordered_set;
    }

    auto merge_small_to_large(std::unordered_set<u32> &large, std::unordered_set<u32> &small)
        -> void {
