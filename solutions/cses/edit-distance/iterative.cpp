...
// Submission URL: https://cses.fi/problemset/result/16477398/
// Verdict: ACCEPTED
...
auto levenshtein_distance(const std::vector<char> &a, const std::vector<char> &b) -> i32 {
    auto prev_row = std::vector<i32>(std::ssize(b) + 1);
    auto curr_row = std::vector<i32>(std::ssize(b) + 1);

    for (auto j = i32{0}; j <= std::ssize(b); j += 1) {
        curr_row[j] = j;
    }

    for (auto i = i32{1}; i <= std::ssize(a); i += 1) {
        std::swap(curr_row, prev_row);
        curr_row[0] = i;
        for (auto j = i32{1}; j <= std::ssize(b); j += 1) {
            curr_row[j] = a[i - 1] == b[j - 1]
                              ? prev_row[j - 1]
